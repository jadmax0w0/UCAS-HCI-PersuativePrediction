...
def _get_dataset_features(
    ...
    if isinstance(cialdini_extractor, CialdiniFeatureExtractor):
        p_feat_cialdini = cialdini_extractor.extract(df["p"].values.tolist())
    else:
        p_feat_p, p_feat_n = Cialdini.load_from_anno_file(cialdini_extractor, partition=partition)
        ...
        p_feat_cialdini = p_feat_cialdini[:len(df)]
    
    # 10 dim feats
    if partition == 'train' and not dim10_extractor.trained():
        dim10_extractor.train()
    p_feat_dim10 = dim10_extractor.extract(df["p"].values.tolist())
    
    # Bert feats
    if partition == 'train' and not bert_extractor.trained():
        bert_extractor.train()
    p_feat_bert = bert_extractor.extract(df["p"].values.tolist())
    o_feat_bert = bert_extractor.extract(df["o"].values.tolist())

    # Concat feats
    x = np.concat([p_feat_cialdini, p_feat_dim10, p_feat_bert, o_feat_bert], axis=-1)
...
def train_model(
    ...
    print(">>> Load and vectorize training dataset")
    data_path = "./dataset/train.jsonl" if data_path is None else data_path
    x_train, y_train = _get_dataset_features(
        partition='train',
        data_path=data_path,
        cialdini_extractor=cialdini_extractor,
        dim10_extractor=dim10_extractor,
...
def eval_model(
    ...
    ## Load and vectorize eval data
    print(">>> Load and vectorize evaluation dataset")
    data_path = "./dataset/val.jsonl" if data_path is None else data_path
    x_test, y_test = _get_dataset_features(
        partition='val',
        data_path=data_path,
        cialdini_extractor=cialdini_extractor,
        dim10_extractor=dim10_extractor,
